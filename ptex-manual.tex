...
\begin{itemize}
 ...
 \item 最上位ビットが1の場合，そのバイトで始まる列について
    ...
    %
    % 内部コード Unicode (uptex) では
    % 「0..0x10FFFF」よりも広い「TeX が扱える 0 以上の整数値全体」
    % が文字コードとして有効である．
    % upTeX 内部では 0x110000 以上の文字コードを扱うことができ，
    % DVI 出力時には「文字コード mod 0x110000」で書き込まれる．
    % これは 0x10FFFF 以下のカテゴリーコードや禁則ペナルティの
    % しがらみの影響を受けない目的で otf パッケージで使われる：

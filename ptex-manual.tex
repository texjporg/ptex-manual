...
(preloaded format=ptex)
\end{verbatim}
のように表示される（入出力ファイルの文字コードと内部コードがともにShift-JISである例）．
...
入力は\emph{「7ビットASCII文字集合」に「あるファイルの文字コードで
...
\footnote{実は上限が0x10FFFFというのは嘘である：
0x110000以上も有効な文字コードであり，特殊な用途で利用される．
% 「0..0x10FFFF」よりも広い「TeX が扱える 0 以上の整数値全体」
...
% [TODO] 後の和文文字トークンの説明で触れる：
%   * 和文文字トークンは 128-- の文字コードが有効で，0--127 はあり得ない
%   * 和文文字ノードは 0--127 の文字コードもあり得る（\kchar で生成）

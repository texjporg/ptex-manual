...
\upTeX は\pTeX と事情が異なり，さらに既定の内部Unicodeと
内部レガシー（2023-06-01以降の\pLaTeX で利用）でも異なるので，ここで説明する．

まず，\upTeX の内部レガシー（EUC-JPまたはShift-JIS）の方は\pTeX に近い．
% pLaTeX がこの状態なので記載必要！
\begin{itemize}% [TODO] aminophen 2023-08-24: 記載してみた：正しいか？
  \item 文字集合は「7ビットASCII文字集合」に「あるファイルの文字コードで
    エンコードされたJIS~X~0208の文字集合」を加えたもの．（\pTeX と同様）
  \item 和文文字と欧文文字の区別は以下の規則による：
%
\begin{itemize}
 \item 7ビットASCII文字集合は欧文文字として扱われる．
 \item 最上位ビットが1の場合　…中略…　
    内部コードへの変換を試みる（この点は\pTeX と同様）．
    内部コードのパターンに合えば「変換後の文字コードを持つ一文字」
    として扱われ（それが和文扱いになるか欧文扱いになるかは
    |\kcatcode|に依存），合わなければ「8ビット欧文文字のバイト列」
    として扱われる．
...
%
 \item 「|^^|記法」は常に欧文扱いされる．（\pTeX と同様）
  \item \code{ptexenc}ライブラリによる特殊加工も\pTeX と同様．
\end{itemize}

\upTeX の既定（内部Unicode）においては… [TODO]
\begin{itemize}% [TODO] aminophen 2023-08-24: 記載してみた：未完成
  \item 文字集合はUnicode全体．
  \item 和文文字と欧文文字の区別は…．% [TODO]
  \item |^^|記法が常に欧文扱いされる点は\pTeX と同様．
  \item \code{ptexenc}ライブラリによる特殊加工については，
    内部Unicodeの場合以下となる：
   \begin{itemize}
    \item Unicode多対一変換は行われない．
    \item BOMの無視は行う．
    \item 結合濁点・半濁点の合字処理は行う（\TeX~Live 2016以降）．
    \item 範囲外の文字はないので|^^ab|形式への変換は起こらない．
   \end{itemize}
\end{itemize}

いずれの場合も，\upTeX は
オリジナルの\TeX や\pdfTeX などの欧文\TeX とは入力に関して必ずしも
互換でないが，\pTeX と比較すれば幾分差異が軽減されている．

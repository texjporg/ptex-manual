...
についてまとめたものである．
オリジナルのASCII~\pTeX とは動作が異なる点もあるので注意されたい．
...
\begin{cslist}
  ...
  \footnote{%
	...
	内部コードがShift-JISのときは
	$2n-1$区・$2n$区($1≦n≦47$)区は同一のカテゴリーコードを持つことになる．
  }．
  
  \pTeX においては，|\kcatcode|を初期状態
  （1,~2, 7--15, 85--94区の文字の|\kcatcode|は18，3--6区の文字は17，
  16--84区の文字は16）から変更することは想定されていない．
...
\begin{itemize}
 ...
 \item |\nullfont|は全ての文字が未定義な「空フォント」を指すが，
  ...
  % [TODO] これは仕様とみなしてよい？
  % 現状では，tikzpicture 環境のように|\selectfont|を|\nullfont|に

...
% 他の 3 つの文字コードと明らかに区別ができるため。
% 実際には，他の文字コードのファイルの途中に JIS の文字列が混ざっていても
% 全て正常に読み込まれる。
...
\begin{itemize}
 ...
 \item 最上位ビットが1の場合，\pTeX はそのバイトで始まる列について
  ...
  \end{description}
    である．この
    内部コードのパターンに合えば「変換後の文字コードを持つ一つの和文文字」
    として扱われ，合わなければ「8ビット欧文文字のバイト列」として
    扱われる\footnote{例えば，入力ファイルの文字コードがUTF-8の場合，
    JIS~X~0208にない文字は一旦|^^ab|形式への変換を経てUTF-8のバイト列と
    して扱われる．}．
\end{itemize}
...
\begin{itemize}% [TODO] aminophen 2023-08-24: 記載してみた：正しいか？
...
\begin{itemize}
 \item 7ビットASCII文字集合は欧文文字として扱われる．
 \item 最上位ビットが1の場合，\upTeX は …中略…　
    内部コードへの変換を試みる（同一コードなら無変換）．
    内部コードのパターンに合えば「変換後の文字コードを持つ\emph{一文字}」
    として扱われ，\emph{それが和文扱いか欧文扱いかは
    \.{kcatcode}に依存する（詳細は\ref{sec:uptex_kcat}節）．}
    合わなければ「8ビット欧文文字のバイト列」として扱われる．
    % pTeX で和文固定の文字（例えば "あ"）も
    % upTeX -kanji-internal=euc/sjis なら \kcatcode=15 で欧文扱いできる．
    % ただし入力ファイルの文字コードによらず内部コードにより
    % 結果は ^^a4^^a2 (euc) / ^^82^^a0 (sjis) になる．
\end{itemize}
%
 \item 「|^^|記法」は常に欧文扱いされる．（\pTeX と同様）
 \item \code{ptexenc}ライブラリによる特殊加工も\pTeX と類似だが，
   変換方向によって少々異なる．
%
\begin{itemize}
 ...
 \item 範囲外の文字の|^^ab|形式への変換：内部レガシーでは\pTeX と同様．
  \emph{内部Unicodeでは，文字集合がUnicode全体なので
  範囲外の文字は存在せず，従って}|^^ab|\emph{形式への変換は起こらない．}
\end{itemize}
...
カテゴリーコードのいずれかを割り当てる．
和文文字のカテゴリーコードを取得・設定するプリミティブが用意されている．
...
（結局，16と17は\pTeX の内部処理においては全く等価と言える）：
\begin{itemize}
  ...
  \item 後で説明する|\jcharwidowpenalty|は，カテゴリーコードの値が16,~17の和文文字の前にのみ
    挿入されうるもので，値が18の和文文字の前には挿入されない．
...
というのも，\emph{\pTeX では（\upTeX と異なり）和文文字トークンに
カテゴリーコードの情報は保存されず，
和文文字が処理対象となるたびにカテゴリーコードの値が再取得される}ためである．
...
\upTeX においては\.{kcatcode}が\pTeX から大きく変更されており，
和文文字と欧文文字の区別も変更できるよう拡張されている
...
\begin{cslist}
  ...
  内部Unicodeの\upTeX では，和文文字のカテゴリーコード(|\kcatcode|)は
  概ねUnicodeのブロックごと（※一部のブロックは分割してある）に値が設定可能である．
  内部レガシーの\upTeX では，\pTeX と同様に
  JISコードでいう区ごとに値が設定可能．
\end{cslist}

まず，和文文字に可能なカテゴリーコード(\.{kcatcode})の値として，\pTeX 互換の
16~($\mathit{kanji}$), 17~($\mathit{kana}$), 18~($\mathit{other\_kchar}$)のほか，
19~($\mathit{hangul}$)が追加されている．
カテゴリーコード19は，その和文文字直後の改行を欧文同様に空白とみなす
という点が特別であり，それ以外の点ではカテゴリーコード16,~17の和文文字と同様である．
さらに，特別な値として\.{kcatcode}に15~($\mathit{not\_cjk}$)を指定すると，
それは8ビット欧文文字のバイト列として扱われる．

...
\patchcmd\quote{\list{}}{\par\medskip\list{\topsep\z@}}{}{}
\patchcmd\endquote{\endlist}{\endlist\medskip}{}{}
...
\begin{dangerous}
  ...
  されるものであるので，\.{lastskip}などで取得することはできない．
...
\csitem[\.{inhibitxspcode}<16-bit number>=<0--3>]
  コード番号が<16-bit number>の和文文字の周囲に\.{xkanjiskip}が
  挿入可能が否かを0--3の値で指定する．それぞれの意味は次の通り：
  \begin{description}
...
\begin{dangerous}
 ...
 （ただノードが\.{kanjiskip}や\.{xkanjiskip}の代わりに長さ0のグルーを表すだけ）．
  ...
  これにより，例えば\.{noautoxspacing}状況下で「\hbox{\noautoxspacing\verb+あa+}」と
...
\end{verbatim}
\end{itemize}
% ======== inhibit_flag_list branch
\csitem[\.{disinhibitglue}]
  この命令が実行された位置において，\.{inhibitglue}を無効化する．
% ========
\end{cslist}
\begin{dangerous}
  p\LaTeX~2017-10-28以降では，\.{inhibitglue}の短縮として\.{\char`\<}が次のように
...
% ======== inhibit_flag_list branch
\pTeX~p3.*.*では，\.{inhibitglue}の有効範囲について大きな変更が行われた．
これより前の\pTeX では\.{inhibitglue}が有効か否かを表すフラグは一つの
グローバル変数であったが，\pTeX~p3.*.*では\.{spacefactor}の扱いに倣い，
各リストごとにフラグを持つようになり，以下のような仕様となった：

\begin{itemize}
 \item 和文文字トークン$t$の直後にある\.{inhibitglue}は，
  $t$の直後のJFMグルーの挿入を抑止する．
 \item リストに文字，ボックス，罫線，insertionなどのノードを追加した場合には
  \.{inhibitglue}は無効状態へとリセットされる．
 \item ノードを生成しない命令（\.{relax}や\>|{}|，それに代入命令など）によっては
  \.{inhibitglue}の状態はリセットされない．
\end{itemize}
...
\begin{dangerous}
  ...
  によって「）」と「・」の間に空白が入らないことが理解できるだろう．
  \begin{enumerate}
	\item 「）」の直後にある\.{inhibitglue}のため，本来ならば
	（「）」と「\.{relax}」の間に入る）二分空きは入らない．
	\item \.{relax}によって\.{inhibitglue}は無効化されない．
	\item 従って，本来ならば「\.{relax}」と「・」の間に挿入される
	四分空きも，結果的には挿入されないこととなる．
	\item 「・」がノードとして現在のリストに追加されることで
	\.{inhibitglue}は無効状態にリセットされる．
	従って，「・」と「あ」の間にはJFM通り四分空きがきちんと挿入される．
  \end{enumerate}
\end{dangerous}
% ========
...
\end{verbatim}
  ...
  \begin{itemize}
    ...
    \item recent contributionの中身にもボックス，罫線，
      insertionはない．
  \end{itemize}
...
\end{cslist}
\begin{dangerous}
 \.{discretionary}命令では\.{discretionary\{<pre>\}\{<post>\}\{<nobreak>\}}と3つの引数を指
...
% これは jtexdoc.tex によると日本語化されていないと明記あり
% 他にもあるか？
% 2016 年以前の「上位バイト」な挙動も合わせて書くか？
\.{catcode}
\.{sfcode}
\.{mathcode}
\.{delcode}

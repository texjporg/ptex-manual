...
\emph{内部コード}に変換されて内部処理に回される．
内部コードはWindowsではShift-JIS，それ以外ではEUC-JPが既定であるが，
起動時のオプション|-kanji-internal|（後述）によって制御できる．

入力ファイルの文字コードは次の順番で決まる（\code{ptexenc}による）．
処理はファイルの先頭行を読むときに行われる．
...
\begin{enumerate}
 ...
 \item 以上で決まらない場合は既定の文字コード\footnote{\TeX~Live 2018以降は
   全てのプラットフォームでUTF-8である．\TeX~Live 2017以前は
   WindowsではShift-JIS，それ以外ではUTF-8であった．}を使う．
\end{enumerate}
% 以上の記述は阿部さんのブログを基に作成．

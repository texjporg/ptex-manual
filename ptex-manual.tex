...
禁則テーブルへの登録には``|{}|''によるグルーピング規則が適用され，
禁則テーブルからの登録の削除は，「グローバルな代入（|\global|使用）
あるいは最も外側のグループ（\eTeX でいうところの$|\currentgrouplevel|=0$）
で，ペナルティ値0を設定する」ことによって行われる．
...
\csitem[\.{inhibitxspcode}<16-bit number>=<0--3>]
  ...
  未登録時は3（両側許可）であるとみなされ，またグローバルな代入あるいは
  最も外側のグループで3が指定されるとテーブルからの削除が行われる．
\end{cslist}
\begin{dangerous}

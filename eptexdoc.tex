...
\end{verbatim}
 ...
 \csitem[\.{tracingstacklevels} (integer)]
  ...
  % 実は，このchange file (tracingstacklevels.ch)は素の\pTeX にも適用されている．
 ...
 \csitem[\.{partokenname} <control sequence>]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item 水平モード中で\TeX が垂直モード用の命令
      (\.{vskip}, \.{hrule}, \.{unvbox}, \.{unvcopy}, \.{halign}, \.{end}, \.{dump})
      に出くわしたとき，その直前
    \end{itemize}
    このことを利用して，\LaTeXe~2021-06-01などでは「段落終了時のフック」を
    実現させるため，\.{par}を再定義している．
...
\end{verbatim}
  ...
  \medskip
  さて，今回の命令\.{partokenname}は，上記で述べた特別な役割を他の制御綴に移す効果を持つ．
  \begin{quote}
	\verb+\partokename \hoge+
  \end{quote}
  を実行した際に起こる状況で説明すると，
  \begin{itemize}
    ...
    逆に\.{hoge}が許容されなくなる．
  \end{itemize}
  \.{partokenname}の効果は常にグローバルである．
  
 \csitem[\.{partokencontext} (integer)]
  \TeX が\.{par}\footnote{あるいは\.{partokenname}により代わりに別の制御綴になっているかもしれない．}を
  自動挿入する箇所を制御する内部整数であり，既定値は0である．

  自動挿入箇所は次の通り：
  \begin{description}
    \item[いつでも挿入] \TeX82での自動挿入箇所（\.{partokenname}の箇所で説明した）．

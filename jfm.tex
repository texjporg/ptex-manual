...
ZR氏によって開発されたPerlスクリプトである．主な機能は以下のとおり：
\begin{itemize}
  \item 和文の仮想フォント（VFとJFMの組）のファイルを
    別の名前で複製する機能（多書体化などに有用）
  \item 和文の仮想フォント（VFとJFMの組）を独自仕様の
    テキスト形式（ZVP形式）と相互変換する機能（和文VFの新規作成や編集に有用）
\end{itemize}
詳細は公式ドキュメントを参照してほしい．
